...
export function NotificationFeed(selector, options) {
    try {
        ...
        this.fetchFeed = async () => {
            try {
                const { domain, apiKey, token } = container.options.authentication;

                const { offset, limit } = container;
        ...
        container.addEventListener('scroll', () => {
            const endOfPage = container.clientHeight + container.scrollTop >= container.scrollHeight;
            if (endOfPage) {
                container.offset += 10;
