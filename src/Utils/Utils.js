...
export function getSafetynetUserSelection(node) {
    var nodes = node.getElementsByClassName('autoql-vanilla-chata-safetynet-select');
    var selections = [];
...
export function getPivotColumnArray(json, options, _data) {
    ...
    var values = [];
    var firstColName = '';
    var name = json['data']['columns'][1]['display_name'] || json['data']['columns'][1]['name'];
...
export async function getLocalStream() {
    ...
    try {
        ...
        console.log('successfully connected to audio.');
    } catch (error) {
        console.error(error);
    }
    return;
...
export const supports2DCharts = (columns) => {
    const amounts = getColumnTypeAmounts(columns);
        ...
        if (supportsRegularPivotTable(columns)) {
            ...
            if (dateColumn) {
                if (
                    ...
                    const uniqueYears = [];
                    rows.forEach((row) => {
                        const year = formatData(row[dateColumnIndex], dateColumn, {
                            dataFormatting: {
...
export function closeAutocompleteObjects() {
    var list = document.querySelectorAll('.autoql-vanilla-data-explorer-autocomplete');
...
export function closeAllPopups() {
    var list = document.querySelectorAll('.autoql-vanilla-select-popup');
    for (var i = 0; i < list.length; i++) {
        list[i].close();
    }
}
...
export function closeAllToolbars() {
    ...
    var popovers = document.querySelectorAll('.autoql-vanilla-popover');
