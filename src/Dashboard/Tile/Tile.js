...
import { QUERY, NOTEBOOK, TILE_RUN_QUERY, DASHBOARD_DELETE_ICON, SPLIT_VIEW, SPLIT_VIEW_ACTIVE } from '../../Svg';
...
export function Tile(dashboard, options) {
    ...
    var inputContainer1 = new InputContainer(['chata-rule-input', 'dashboard-tile-left-input-container']);

    var inputContainer2 = new InputContainer(['chata-rule-input', 'dashboard-tile-right-input-container']);

    var queryInput = new ChataInput(
        'input',
        {
    ...
    var queryInput2 = new ChataInput(
        'input',
        {
    ...
    item.tileTitle.textContent = options.title || item.options.query || strings.untitledTile;
    item.inputQuery.value = options.query;
    item.inputTitle.value = options.title || item.options.query;
    item.responseWrapper = responseWrapper;
    item.dashboard = dashboard;

    item.getValues = () => {
        const { query, title, key, isSplit } = item.options;
    ...
    item.stopEditing = () => {
        ...
        item.tileTitle.textContent = item.inputTitle.value || item.inputQuery.value || strings.untitledTile;
        deleteButton.style.visibility = 'hidden';
