...
export function QueryInput(selector, options) {
    ...
    chataBarContainer.applyStyles = () => {
        const themeStyles = chataBarContainer.options.themeConfig.theme === 'light' ? LIGHT_THEME : DARK_THEME;
        themeStyles['accent-color'] = chataBarContainer.options.themeConfig.accentColor;
    ...
    chataBarContainer.addEventListener('click', function (e) {
        if (e.target.classList.contains('suggestion-renderer')) {
            ...
            chataBarContainer.sendMessageToResponseRenderer(e.target.textContent, 'user');
        }
    });
    ...
    var chataIcon = htmlToElement(CHATA_ICON);
    ...
    chataBarContainer.style.setProperty('--chata-drawer-font-family', chataBarContainer.options.fontFamily);
    ...
    chataBarContainer.sendDrilldownClientSide = (json, indexValue, filterBy) => {
        ...
        var parent = chataBarContainer.getElementsByClassName('autoql-vanilla-chat-bar-text')[0];
    ...
    chataBarContainer.sendDrilldownMessage = (json) => {
        ...
        let opts = mergeOptions([chataBarContainer.options, responseRenderer.options]);
    ...
    chataBarContainer.onRowClick = (e, row, json) => {
        ...
        if (groupableCount > 0) {
            ...
            responseRenderer.options.onDataClick(clickedData, json['data']['query_id']);
    ...
    chataBarContainer.sendMessageToResponseRenderer = function (value, source, selections = undefined) {
        var responseRenderer = this.responseRenderer;
        ...
        const URL_SAFETYNET = chataBarContainer.options.authentication.demo
            ? `https://backend.chata.ai/api/v1/safetynet?q=${encodeURIComponent(value)}&projectId=1`
            : `${
                  chataBarContainer.options.authentication.domain
              }/autoql/api/v1/query/validate?text=${encodeURIComponent(value)}&key=${
                  chataBarContainer.options.authentication.apiKey
              }&customer_id=${chataBarContainer.options.authentication.customerId}&user_id=${
                  chataBarContainer.options.authentication.userId
              }`;
        ...
        ChataUtils.safetynetCall(
            ...
            async function (jsonResponse) {
                ...
                if (
                    ...
                    chataBarContainer.options.autoQLConfig.enableQueryValidation &&
                    ...
                    var node = createSafetynetContent(suggestionArray, () => {
                        ...
                        var selections = getSafetynetUserSelection(responseRenderer);
                        chataBarContainer.sendMessageToResponseRenderer(words.join(' '), 'validation', selections);
                    });
                    ...
                    var response = await apiCall(value, chataBarContainer.options, source, selections);
    ...
    chataBarContainer.refreshView = (jsonResponse, text = '') => {
        var responseRenderer = chataBarContainer.responseRenderer;
        // ChataUtils.responses[responseRenderer.dataset.componentid] = jsonResponse;
        responseRenderer.innerHTML = '';
        ...
        if (jsonResponse['reference_id'] === '1.1.430') {
            ...
            const path = ChataUtils.getRecommendationPath(chataBarContainer.options, text.split(' ').join(','));
            ChataUtils.safetynetCall(
                path,
                function (response) {
                    var wrapper = document.createElement('div');
                    var rows = response['data']['items'];
                    ChataUtils.createSuggestions(wrapper, rows, 'autoql-vanilla-chata-suggestion-btn-renderer');
        ...
        if (displayType == 'table') {
            ...
            if (cols[0]['name'] == 'query_suggestion' && responseRenderer.options.supportsSuggestions) {
                ...
                if (cols[0]['name'] == 'Help Link') {
                    ...
                    var data = formatData(rows[0][0], cols[0], responseRenderer.options);
                    responseRenderer.innerHTML = `
                        <div>${data}</div>
                    `;
                }
